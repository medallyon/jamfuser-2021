...
void FInputMappingGroup::RemoveRedundantMappings(const FInputMappingGroup& BaseMappingGroup)
{
	...
	for(FConfigAxisKeyMapping& Axis : AxisMappings)
	{
		if(BaseMappingGroup.AxisMappings.Contains(Axis))
		{
			...
			{
				if(UnboundAxisMapping.AxisName == Axis.AxisName && GetDefault<UAutoSettingsConfig>()->IsAxisKey(UnboundAxisMapping.Key))
				{
	...
	{
		const FGameplayTag KeyGroup = GetDefault<UAutoSettingsConfig>()->GetKeyGroup(UnboundAction.Key);
	...
	{
		const FGameplayTag KeyGroup = GetDefault<UAutoSettingsConfig>()->GetKeyGroup(UnboundAxis.Key);
