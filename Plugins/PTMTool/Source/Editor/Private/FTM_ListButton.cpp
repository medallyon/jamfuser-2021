...
const FSlateBrush* SFTM_ListButton::GetTypeBrush() const {
	const FSlateBrush* Brush = FEditorStyle::GetBrush(TEXT("Kismet.Status.Unknown.Small"));
	if (!FInfo.Graph.IsValid()) {return Brush;}
	//
	if (GetFunctionType()==GT_Function) {Brush=FEditorStyle::GetBrush(TEXT("Kismet.AllClasses.FunctionIcon"));}
	if (GetFunctionType()==GT_Macro) {Brush=FEditorStyle::GetBrush(TEXT("GraphEditor.Macro_16x"));}
	//
	return Brush;
}
