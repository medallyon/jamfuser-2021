...
void FPTMTool::StartupModule() {
	...
	PTM_Extender->AddMenuExtension("FileLoadAndSave",EExtensionHook::After,TSharedPtr<FUICommandList>(),FMenuExtensionDelegate::CreateStatic(&FPTMTool::PTM_ExtendMenu));
	LevelEditorModule.GetMenuExtensibilityManager()->AddExtender(PTM_Extender);
	//
	FTM_Extender = MakeShareable(new FExtender());
	FTM_Extender->AddMenuExtension("FileLoadAndSave",EExtensionHook::After,TSharedPtr<FUICommandList>(),FMenuExtensionDelegate::CreateStatic(&FPTMTool::FTM_ExtendMenu));
